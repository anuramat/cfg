local custom = { headerChar = "~" }

local utils = require("utils")

---@diagnostic disable-next-line: duplicate-set-field
function custom.CreateCommentHeader()
  vim.ui.input({ prompt = "Header text:" }, function(text)
    ...
    local width = tw - utils.get_indent()

    local header = "%s"
    local char = custom.headerChar

    local commentString = vim.api.nvim_buf_get_option(0, "commentstring")
    commentString = utils.trim(commentString)
